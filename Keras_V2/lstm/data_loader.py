...
class BaseGenerator(Sequence):
    def __init__(self, config):
        ...
        for i in range(len(self.train_paths) // self.seq_len):
            temp_in = self.train_paths[i*self.seq_len : (i+1)*self.seq_len]
            temp_out = self.train_outputs[i*self.seq_len : (i+1)*self.seq_len]
        ...
        paths, y = [], []
        for i in range(len(self.cv_paths) // self.seq_len):
            temp_in = self.cv_paths[i*self.seq_len : (i+1)*self.seq_len]
            temp_out = self.cv_outputs[i*self.seq_len : (i+1)*self.seq_len]
            paths.append(temp_in)
            y.append(temp_out)
...
class TrainGenerator(BaseGenerator):
     ...
     def __getitem__(self, idx):
        ...
        #Y = np.zeros((self.batch_size, self.seq_len, 7))
        ...
        for i in range(self.batch_size):
            ...
            for j in range(self.seq_len):
                ...
                X[i][j] = img
                #Y[i][j] = to_categorical( outputs[j], num_classes=7 )                  
            Y[i] = to_categorical( outputs[self.seq_len-1], num_classes=7 )
        return (X, Y)
